	...
	public List<MemberVO> listMembers() {
		...
		try {
			...
			while (rs.next()) {
				...
				String confirmPw = rs.getString("confirmPw");
				String hp=rs.getString("hp");
				String email = rs.getString("email");
				String sex = rs.getString("sex");
				Date birth=rs.getDate("birth");
				String region = rs.getString("region");
				int shoesSize = rs.getInt("shoesSize");
				
				MemberVO memberVO=new MemberVO(id, name, pw, confirmPw, hp, email, sex, birth, region, shoesSize);
	...
	*/
	...
	public void joinMember(MemberVO memberVO) {
		...
		try {
			...
			String confirmPw = memberVO.getConfirmPw();
			String hp = memberVO.getHp();
			String email = memberVO.getEmail();
			String sex = memberVO.getSex();
			Date birth = memberVO.getBirth();
			String region = memberVO.getRegion();
			int shoesSize = memberVO.getShoesSize();
			String query="insert into memberlist(id, name, pw, confirmPw, hp, email, sex,  birth, region, shoesSize) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			pstmt=conn.prepareStatement(query);
			pstmt.setString(1, id);
			pstmt.setString(2, name);
			pstmt.setString(3, pw);
			pstmt.setString(4, confirmPw);
			pstmt.setString(5, hp);
			pstmt.setString(6, email);
			pstmt.setString(7, sex);
			pstmt.setDate(8, birth);
			pstmt.setString(9, region);
			pstmt.setInt(10, shoesSize);
			pstmt.executeUpdate();
			pstmt.close();
			conn.close();
		} catch (Exception e) {
